...
if __name__ == '__main__':
    ...
    comp_path = './dataset/test/comp/'
    raw_save_path = './dataset/processed/test/raw/'
    comp_save_path = './dataset/processed/test/comp/'
    ...
    video_list = glob.glob(raw_path + "*.yuv")
    # video_list = ['./dataset/test_18/raw/KristenAndSara_1280x720_600.yuv']
    for c in video_list:
        ...
        createpath(raw_save_path + filename + '/')
        createpath(comp_save_path + filename + '/')
        ...
        try:
            ...
            [y_lq,u_lq,v_lq] = read_YUV420(comp_path + gt_name + '.yuv', vid_height, vid_width, frame_total)
            ...
            for index in range(frame_total-1):
                ...
                if not os.path.exists(raw_save_path + filename + '/' + filename + '_' + str(index) + '.png'):
                    if channal_num==3:
                        cv2.imwrite(raw_save_path + filename + '/' + filename + '_' + str(index) + '.png', rgb_gt)
                        # np.save(savePath_GT+filename+'/'+filename+'_'+str(index)+'.npy',rgb_gt)
                    elif channal_num==1:
                        cv2.imwrite(raw_save_path + filename + '/' + filename + '_' + str(index) + '.png', temp_y_gt)

                if not os.path.exists(comp_save_path + filename + '/' + filename + '_' + str(index) + '.png'):
                    if channal_num==3:
                        cv2.imwrite(comp_save_path + filename + '/' + filename + '_' + str(index) + '.png', rgb_lq)
                        # np.save(savePath_RAnof+filename+'/'+filename+'_'+str(index)+'.npy',rgb_lq)
                    elif channal_num==1:
                        cv2.imwrite(comp_save_path + filename + '/' + filename + '_' + str(index) + '.png', temp_y_lq)
